...
final class TM_EPO_HELPER_base {
	...
	public function sum_array_values( $epos = array(), $include_variation_prices = FALSE ) {
		...
		/*foreach ($r as $key => $value) {
			if ($include_variation_prices && is_array($value)){
				foreach ($value as $k1 => $v1) {
					$r[$key][$k1]=$v1;//apply_filters('wc_epo_get_current_currency_price',$v1,'');
				}
			}else{
				$r[$key]=$value;//apply_filters('wc_epo_get_current_currency_price',$value,'');
			}
		}*/
