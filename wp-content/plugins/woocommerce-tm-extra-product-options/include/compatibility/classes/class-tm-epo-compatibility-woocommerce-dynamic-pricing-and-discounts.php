...
final class TM_EPO_COMPATIBILITY_woocommerce_dynamic_pricing_and_discounts {
	...
	public function cart_item_price( $item_price = "", $cart_item = "", $cart_item_key = "" ) {
		...
		$price = TM_EPO()->get_price_for_cart( FALSE, $cart_item, "", NULL, 0, $cart_item['quantity'] );

		// Format price to display
		$price_to_display = $price;
		if ( TM_EPO()->tm_epo_cart_field_display == "advanced" ) {
			$original_price_to_display = TM_EPO()->get_price_for_cart( $cart_item['tm_epo_product_original_price'], $cart_item, "", NULL, 0, $cart_item['quantity'] );
			if ( TM_EPO()->tm_epo_dpd_enable == "yes" ) {
				...
				$price_to_display = TM_EPO()->get_price_for_cart( $price, $cart_item, "", NULL, 0, $cart_item['quantity'] );
			} else {
			...
			$original_price_to_display = TM_EPO()->get_price_for_cart( $cart_item['tm_epo_product_price_with_options'], $cart_item, "", NULL, 0, $cart_item['quantity'] );
		}
