(function ( $ ) {
	...
	$.tmEPOAdmin = {
		...
		add_events: function () {
			...
			/*$(document).on("click", ".btitle .tm-element-label", function() {
			 var t=$(this);
			 $.tmEPOAdmin.current_edit_label=t;
			 t.hide().next(".tm-internal-name").focus();
			 });*/
			...
			$( document ).mouseup( function ( e ) {
				...
				if ( ! container.is( e.target ) // if the target of the click isn't the container...
					...
					//&& (!container.next(".tm-internal-name").is(e.target) || !container.closest(".tm-label-desc").next(".tm-label-desc-edit").find(".tm-internal-name").is(e.target))
...
//                var _this = $(this).prev("input");
		...
		tm_upload: function ( e ) {
			var $this = $( "#temp_for_floatbox_insert" ),
				...
				$use_color_all = $( "#temp_for_floatbox_insert .use_colors" ),
				$use_div_images = $( "#temp_for_floatbox_insert .tm-use-images" ),
				$use_div_colors = $( "#temp_for_floatbox_insert .tm-use-colors" ),
	...
	function tm_display_settings( select ) {
		...
		//var row5 = $( '#tm_epo_options_placement_hook_priority' ).closest( 'tr' );
		//var row6 = $( '#tm_epo_totals_box_placement_hook_priority' ).closest( 'tr' );
	...
	function tm_options_placement_settings( select ) {
		...
		//var row2 = $( '#tm_epo_options_placement_hook_priority' ).closest( 'tr' );
		if ( val == "custom" ) {
			row1.show();
			//row2.show();
		} else {
	...
	function tm_totals_box_placement_settings( select ) {
		...
		//var row2 = $( '#tm_epo_totals_box_placement_hook_priority' ).closest( 'tr' );
		if ( val == "custom" ) {
			row1.show();
			//row2.show();
		} else {
